...
describe(
    ...
    database([Customer], () => {
        ...
        it("processorFieldsDiscounts when deleting", () => {
            ...
            assert.deepEqual(
                subscriptionProcessor.processorFieldsDiscounts(originalDiscounts, discounts),
                expected
            );
        });
        ...
        it("processorFields should map models to braintree data", function() {
            return this.customer.save().then(customer => {
                const fields = subscriptionProcessor.processorFields(
                    customer,
        ...
        it("fieldsDiscounts should map resul data into discount models", () => {
            ...
            const fields = subscriptionProcessor.fieldsDiscounts(
                originalDiscounts,
                resultDiscounts
            );
        ...
        it("fields should map result data into a model", function() {
            ...
            const fields = subscriptionProcessor.fields(
                this.customer,
                originalDiscounts,
            ...
            return subscriptionProcessor
                ...
                .then(customer => {
                    ...
                    sinon.assert.calledWith(
                        ...
                        "event",
                        sinon.match
                            .has("name", "subscription")
                            .and(sinon.match.has("action", "saved"))
                    );
                    sinon.assert.calledOnce(gateway.subscription.create);
                    assert.deepEqual(subscription.processor.toObject(), {
                        id: "gzsxjb",
                        state: "saved",
                    });
                });
            ...
            return subscriptionProcessor
                ...
                .then(customer => {
                    ...
                    sinon.assert.calledWith(
                        ...
                        sinon.match
                            .has("name", "subscription")
                            .and(sinon.match.has("action", "canceling"))
                    );
            ...
            return subscriptionProcessor
                ...
                .then(customer => {
                    ...
                    sinon.assert.calledWith(
                        ...
                        sinon.match
                            .has("name", "subscription")
                            .and(sinon.match.has("action", "canceled"))
                    );
                    sinon.assert.calledOnce(gateway.subscription.cancel);
            ...
            return subscriptionProcessor
                ...
                .catch(error => {
                    ...
                    sinon.assert.neverCalledWith(
                        processor.emit,
                        "event",
                        sinon.match.has("action", "canceled")
                    );
            ...
            return subscriptionProcessor
                ...
                .then(customer => {
                    ...
                    sinon.assert.neverCalledWith(
                        processor.emit,
                        "event",
                        sinon.match.has("name", "subscription")
                    );
                });
        });
